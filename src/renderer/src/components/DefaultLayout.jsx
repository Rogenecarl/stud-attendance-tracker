  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <aside 
        ...
        } bg-white border-r border-gray-100 flex flex-col fixed h-full transition-all duration-300 ease-in-out`}
      >
        {/* Logo section */}
        <div className={`flex items-center gap-3 px-6 py-5 ${isSidebarCollapsed ? 'justify-center' : ''} relative border-b border-gray-100`}>
          ...
          <button
            ...
            className="absolute -right-4 top-1/2 -translate-y-1/2 w-8 h-8 bg-white border border-gray-100 rounded-full shadow-sm flex items-center justify-center hover:bg-gray-50 transition-colors duration-200 z-50"
          >
            <svg
        ...
        <nav className="flex-1 px-3 py-4">
          {navigationItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`flex items-center gap-3 px-3 py-3 rounded-xl mb-1 transition-colors duration-200 ${isActive(item.path)}`}
            >
              {item.icon}
